...
const SKILLS = [
  { name: 'ux-method-advisor', label: 'UX Method Advisor · 全包',   slash: '/ux-method-advisor' },
...
function install({ force, toolFilter, cwd }) {
  ...
  p(`${C.gray}65 UX Methods · 6 Skills · Real Case Studies · EN/繁中${C.reset}`);
  p('');
  ...
  if (totalInstalled > 0) {
    ...
    p(`  ${C.gray}"我接了一個 revamp project，該怎麼做？"${C.reset}`);
